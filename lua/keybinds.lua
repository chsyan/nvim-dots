...
cc.add({
	...
	},
	{
		...
		cmd = function()
			...
			-- 	if client.name == 'ltex' and vim.lsp.buf_is_attached(0, client_id) then
...
cc.add({
	{
		desc = "Latex build/compile",
		cmd = vim.cmd.TexlabBuild,
		keys = { "n", "<m-s-c>", noremap },
	},
	{
		desc = "Clean latex build files",
		cmd = "<cmd>!latexclean %<cr>",
	},
}, {
	category = "Latex",
})
